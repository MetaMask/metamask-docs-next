...
```javascript
const countOfLines = (valueGetter as any).current.getValue() const countOfLines = (valueGetter as any).current.getValue()).current.getValue()).current.getValue()).current.getValue()
